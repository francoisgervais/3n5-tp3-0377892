...
import com.example.tp3_0377892.databinding.ActivitySujetVoteBinding
import com.google.android.material.snackbar.Snackbar
import org.depinfo.sujetbd.UtilitaireBD
import org.depinfo.sujetbd.Vote

class SujetVote : AppCompatActivity() {
    private lateinit var binding: ActivitySujetVoteBinding
    private lateinit var actionBarDrawerToggle: ActionBarDrawerToggle

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivitySujetVoteBinding.inflate(layoutInflater)
        //setContentView(R.layout.activity_sujet_vote)
        ...
        val nameField = findViewById<EditText>(R.id.editText)
        ...
        button.setOnClickListener {
            ...
            if (sujetId == -1L || name.isEmpty() || rating == 0) {
                Toast.makeText(this, "Veuillez entrer un nom et une note valide.", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            val existingVoteOptional = service.votePourCeSujetCeVotant(sujetId, name)
            if (existingVoteOptional.isPresent) {
                Toast.makeText(this, "Vous avez déjà voté pour ce sujet.", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            if (name.replace("\\s".toRegex(), "").length < 5) {
                Toast.makeText(this, "Le nom doit contenir au moins 5 caractères imprimables.", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            try {
                val vote = Vote(sujetId = sujetId, nomVotant = name, note = rating)
